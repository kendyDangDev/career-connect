...
export async function GET(
  ...
  try {
    ...
    const stats = await ApplicationTimelineService.getStats(params.applicationId);

    // Check if user has access to view these stats
    if (session.user.role === "CANDIDATE") {
      // Verify this is the candidate's own application
      const timeline = await ApplicationTimelineService.getByApplicationId(params.applicationId);
    ...
    return successResponse(
      stats,
      "Timeline statistics retrieved successfully"
    );
  } catch (error: any) {
