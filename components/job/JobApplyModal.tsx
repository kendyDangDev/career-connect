...
const JobApplyModal: React.FC<JobApplyModalProps> = ({
  ...
  const [selectedCvForPreview, setSelectedCvForPreview] =
    useState<CandidateCv | null>(null);
          ...
          {userCvs.map((cv) => (
            ...
            >
              <TouchableOpacity
                ...
                onPress={() =>
                  setFormData({ ...formData, selectedCvId: cv.id })
                }
              >
                <View
    ...
    <ScrollView className="flex-1 px-4">
      ...
      >
        ...
        <Text className="text-sm text-gray-700 flex-1">
          Tôi đồng ý với các điều khoản và điều kiện, xác nhận thông tin trên là
          chính xác
        </Text>
      </TouchableOpacity>
      ...
      <View className="bg-yellow-50 p-4 rounded-lg">
        <View className="flex-row items-center mb-2">
          ...
          <Text className="font-medium text-yellow-800 ml-2">
            Lưu ý quan trọng
          </Text>
        </View>
  ...
  return (
    ...
    >
      <View className="flex-1 bg-white absolute w-full h-full">
        ...
        {/* Step Indicator */}
        <View className="py-4 bg-white shadow-sm">{renderStepIndicator()}</View>
