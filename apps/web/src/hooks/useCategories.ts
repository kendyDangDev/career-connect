...
export function useCategories(): UseCategoriesReturn {
      ...
      try {
        ...
        if (!query || query.parentId !== 'null') {
          const treeResponse = await fetch(
            '/api/admin/system-categories/categories?parentId=null&includeChildren=true&limit=100',
            {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
  ...
  const createCategory = useCallback(
    async (data: CreateCategoryDto): Promise<boolean> => {
      try {
  ...
  const updateCategory = useCallback(
    async (id: string, data: UpdateCategoryDto): Promise<boolean> => {
      try {
        const response = await fetch(`/api/admin/system-categories/categories/${id}`, {
          method: 'PUT',
          headers: {
