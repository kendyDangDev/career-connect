...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === 'UPDATE_STATUS' && (!body.status || !Object.values(ApplicationStatus).includes(body.status))) {
      return NextResponse.json(
    ...
    if (body.action === 'ADD_RATING' && (body.rating === undefined || body.rating < 1 || body.rating > 5)) {
      return NextResponse.json(
