...
export const createJobAlertSchema = z.object({
  ...
  locationIds: z.array(z.string().cuid())
...
export const updateJobAlertSchema = z.object({
  ...
  (data) => {
    ...
    const hasFilterUpdate = data.keywords !== undefined || 
    ...
    if (hasFilterUpdate && !data.alertName && data.isActive === undefined && data.frequency === undefined) {
      return !!(data.keywords || 
               (data.locationIds && data.locationIds.length > 0) || 
               (data.categoryIds && data.categoryIds.length > 0) || 
               data.jobType || 
...
export const jobAlertQuerySchema = z.object({
  ...
  hasKeywords: z.enum(['true', 'false'])
    .transform(val => val === 'true')
    .optional(),
  
  hasLocations: z.enum(['true', 'false'])
    .transform(val => val === 'true')
    .optional(),
  
  hasCategories: z.enum(['true', 'false'])
    .transform(val => val === 'true')
    .optional(),
  
  sortBy: z.enum(['createdAt', 'alertName', 'lastSentAt', 'frequency'])
    .optional()
    .default('createdAt'),
