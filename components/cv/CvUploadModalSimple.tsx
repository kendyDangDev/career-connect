...
const CvUploadModalSimple: React.FC<CvUploadModalProps> = ({
  ...
  const handleUpload = async () => {
    if (!cvName.trim()) {
      alert.error("Lỗi", 'Vui lòng nhập tên CV');
      return;
    }

    if (!selectedFile) {
      alert.error("Lỗi", 'Vui lòng chọn file CV');
      return;
    }
    ...
    if (!validation.valid) {
      alert.error("Lỗi", validation.error);
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        alert.error("Lỗi", response.error || 'Không thể tải lên CV')
      }
    } catch (error) {
      clearInterval(progressInterval);
      console.error('Error uploading CV:', error);
      alert.error("Lỗi", 'Đã xảy ra lỗi khi tải lên CV. Vui lòng thử lại.');
    } finally {
        ...
        <View className="flex-1 p-4">
          ...
          <View className="mb-6">
            ...
            {selectedFile ? (
              ...
              >
                ...
                <Text className="text-gray-600 font-medium mt-2">Chọn file CV</Text>
          ...
          <View className="bg-blue-50 p-4 rounded-lg mb-6">
            ...
            <Text className="text-blue-700 text-sm">
              ...
              • CV sẽ được lưu trữ an toàn trên cloud
            </Text>
          </View>
        </View>
          ...
          <View className="flex-row">
            ...
            >
              {isUploading ? (
                ...
                <Upload size={20} color={cvName.trim() && selectedFile ? 'white' : '#6B7280'} />
              )}
              <Text className={`ml-2 font-medium ${
                cvName.trim() && selectedFile && !isUploading ? 'text-white' : 'text-gray-500'
              }`}>
